...
import { parsearFechaEnEspanol } from './utils/fechaEspanol';
import { parsearRankingJugador } from './parsers/rankingJugador.parser';
import { parsearSancionados } from './parsers/sancionados.parser';
...
export interface ResumenImportacion {
  ...
  partidosOmitidos: { motivo: string; detalle: string }[];
...
async function procesarFixture(libro: ReturnType<typeof leerLibro>, torneoId: number, resumen: ResumenImportacion) {
  const hoja = buscarHojaPorPatron(libro, /PROX|FIXTURE/i);
  if (!hoja) {
    resumen.advertencias.push('No se encontró hoja de fixture (se esperaba un nombre tipo "Prox Partido").');
    return;
  }

  // Necesitamos el año de la Temporada para convertir "29 de Marzo" -> Date real.
  const torneo = await prisma.torneo.findUnique({ where: { id: torneoId }, include: { temporada: true } });
  if (!torneo) throw new NotFoundError('Torneo', torneoId); // no debería pasar, ya lo validamos en importarExcel

  const { filas, omitidas } = parsearFixture(hoja.matriz);
  resumen.partidosOmitidos.push(...omitidas.map((o) => ({ motivo: o.motivo, detalle: o.detalle })));

  for (const filaFixture of filas) {
    const resuelto = await resolverCategoriaZona(prisma, torneoId, filaFixture.categoriaLabel);
    if (!resuelto) {
      resumen.advertencias.push(
        `Fixture: no se pudo resolver categoría/zona para "${filaFixture.categoriaLabel}" (${filaFixture.equipoLocal} vs ${filaFixture.equipoVisitante}, jornada ${filaFixture.jornada}).`,
      );
      continue;
    }
    ...
    if (!fecha) {
      resumen.advertencias.push(
        `Fixture: no se pudo determinar la fecha calendario (${filaFixture.equipoLocal} vs ${filaFixture.equipoVisitante}, jornada ${filaFixture.jornada}, texto="${filaFixture.fechaTexto}") — se omite, no se puede deduplicar sin fecha.`,
      );
      continue;
    }
    ...
    const data = {
      ...
      fecha,
      jornada: filaFixture.jornada,
      campo: filaFixture.campo,
      cancha: filaFixture.cancha,
      horario: filaFixture.horario,
    };

    // DECISIONES.md #3: reimportar actualiza en vez de rechazar.
    ...
    if (existente) {
      ...
      // La hoja de fixture, en tu archivo real, no trae goles cargados (es
      // el calendario, no el resultado) — entra siempre como PENDIENTE.
      await prisma.partido.create({ data: { ...data, estado: EstadoPartido.PENDIENTE } });
...
// ---------- Sancionados ----------
async function procesarSancionados(libro: ReturnType<typeof leerLibro>, torneoId: number, resumen: ResumenImportacion) {
  const hoja = buscarHojaPorPatron(libro, /SANCIONAD/i);
  if (!hoja) {
    resumen.advertencias.push('No se encontró hoja de sancionados.');
    return;
  }
  const filas = parsearSancionados(hoja.matriz);
  for (const f of filas) {
    const { equipo, creado: ce } = await buscarOCrearEquipo(prisma, f.equipoNombre);
    if (ce) resumen.equiposCreados++;
    const { jugador, creado: cj } = await buscarOCrearJugador(prisma, f.jugadorNombre, equipo.id);
    ...
    // Dedupe: si reimportás el mismo Excel, no duplicamos la misma sanción.
    const existente = await prisma.sancion.findFirst({
      where: { jugadorId: jugador.id, torneoId, tipoTarjeta: f.tipoTarjeta, fechasSuspension: f.fechasSuspension },
    });
    ...
    await prisma.sancion.create({
      data: {
        ...
        fechasSuspension: f.fechasSuspension,
  ...
  try {
    ...
    await prisma.importacionExcel.update({
      ...
      data: {
        ...
        filasActualizadas: resumen.partidosActualizados + resumen.goleadoresActualizados + resumen.figurasActualizadas + resumen.imbatiblesActualizados,
      },
    });
  } catch (error) {
    await prisma.importacionExcel.update({
